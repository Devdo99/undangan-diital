...
const AccountCard: React.FC<AccountProps> = ({ bank, number, owner }) => {
  ...
  return (
    <div className="relative group animate-float">
      <div className="absolute -inset-1 bg-gradient-to-r from-gold/40 to-gold-light/40 rounded-3xl blur opacity-20 group-hover:opacity-60 transition duration-1000" />
...
const Gift: React.FC = () => {
  return (
    <div className="relative h-[100dvh] w-full py-10 px-6 flex flex-col items-center justify-center bg-navy overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm space-y-10">
        {weddingConfig.gifts.map((gift) => (
          <AccountCard key={`${gift.bank}-${gift.acc}`} bank={`BANK ${gift.bank}`} number={gift.acc} owner={gift.owner} />
        ))}
      </div>
