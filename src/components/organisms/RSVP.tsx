...
// Ini yang menyelesaikan error "Property 'brideName' does not exist"
interface RSVPProps {
  brideName: string;
}

const RSVP: React.FC<RSVPProps> = ({ brideName }) => {
  ...
  return (
    <section className="min-h-screen snap-start bg-navy-dark flex flex-col items-center justify-center p-8 relative overflow-hidden">
      
      {/* Ornamen Background */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_100%_0%,rgba(201,168,76,0.05),transparent_50%)]"></div>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_0%_100%,rgba(201,168,76,0.05),transparent_50%)]"></div>
      ...
      <div className="w-full max-w-sm z-10 text-center">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 text-left">
          ...
          <div className="flex flex-col gap-2">
            <label className="font-lato text-[10px] text-gold tracking-[3px] uppercase">Nama Lengkap</label>
            <input 
              type="text" 
              required
              value={name}
          ...
          <div className="flex flex-col gap-2">
            <label className="font-lato text-[10px] text-gold tracking-[3px] uppercase">Konfirmasi Kehadiran</label>
            <select 
              value={attendance}
